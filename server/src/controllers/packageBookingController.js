...
export const getAllBookings = async (req,res)=>{
    try {
        ...
        const bookingQuery = `SELECT BookingID,TouristID,FromDate,NetPayableAmount,BookingDate,cancelstatus FROM BOOKINGS WHERE PACKAGEID = ? AND STATUS = ? AND CANCELSTATUS !=? ORDER BY FromDate ASC, BookingDate DESC`
        const bookingResult = await db.query(bookingQuery,[pkgid,"VERIFIED",2])
